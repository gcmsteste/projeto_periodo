...
public class ControladorMonitorImpl extends ControladorNegocioImpl implements ControladorMonitor {
	...
	public Monitor buscarMonitoriasDeAluno(Aluno aluno) throws NegocioException {
		...
		Monitor monitor = CreatorFabrica.getFabricaDAO().criarMonitorDAO().buscar(aluno.getChavePrimaria());
		return monitor;
	}
	...
	public Monitor cadastrarMonitoria(Monitor monitor) {
		...
		return CreatorFabrica.getFabricaDAO().criarMonitorDAO().salvar(monitor);
	}
	...
	public boolean verificaExistenciaCadastroMonitoria(Monitor monitor) {
		...
		List<Monitor> lista = CreatorFabrica.getFabricaDAO().criarMonitorDAO().listar(builder.toString());
		if (lista.isEmpty()) {
			return false;
		}
		return true;
	...
	public Monitor buscarMonitoria(long chavePrimaria) {
		...
		return CreatorFabrica.getFabricaDAO().criarMonitorDAO().buscar(chavePrimaria);
	}
