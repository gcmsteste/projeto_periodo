...
SELECT 'PadrÃµes de Projeto',
(SELECT PROFESSOR_ID FROM PROFESSOR WHERE PROFESSOR_SIAPE LIKE '1428475'),
(SELECT CURSO_ID FROM CURSO WHERE CURSO_DS LIKE 'AnÃ¡lise de Sistemas') 
FROM DUAL
...
VALUES ( (SELECT ALUNO_ID FROM ALUNO
		 ...
		 ( SELECT DISCIPLINA_ID FROM DISCIPLINA
		   WHERE DISCIPLINA_DS LIKE 'PadrÃµes de Projeto'), current_timestamp() );
		   

INSERT INTO PROJETO_PERIODO.MONITOR (MONITOR_ID, MODALIDADE, PERIODO_ID, DISCIPLINA_ID)
VALUES( (SELECT DS.ALUNO_ID FROM DISCIPLINA_ALUNO AS DS, ALUNO AS A 
      ...
      (SELECT DA.DISCIPLINA_ID FROM DISCIPLINA_ALUNO AS DA, DISCIPLINA AS D
       WHERE D.DISCIPLINA_DS LIKE 'PadrÃµes de Projeto') );
